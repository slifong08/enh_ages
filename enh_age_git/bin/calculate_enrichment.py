...
def main(argv):
    ...
    sid = (sys.argv[1].split("/")[-1]).split("_")[0]
    arch = ((sys.argv[1].split("/")[-1]).split("_")[-1]).split(".")[0]
    target = (sys.argv[2].split("/")[-1]).split(".")[0]
    header = 'Observed\tExpected\tStdDev\tFoldChange\tp-value\tfold_change_95\tfold_change_5\tsid\tdate_time\tarch\tTARGET'
    print(header, target)
    ...
    partial_calcExp = partial(calculateExpected, BedTool(ANNOTATION_FILENAME), BedTool(TEST_FILENAME),\
                              ELEMENT, HAPBLOCK, SPECIES)
    ...
    print(calculateEmpiricalP(obs_sum, exp_sum_list), sid, arch)
    obs_p= calculateEmpiricalP(obs_sum, exp_sum_list)

    if COUNT_FILENAME is not None:
        with open(COUNT_FILENAME, "a") as count_file:

            count_file.write('{}\n'.format("\t".join([obs_p, sid, arch, target])))
