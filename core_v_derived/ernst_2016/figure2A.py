...
def formatdf(f, cell_model):
    ...
    cldf["syn_id"] = cldf.chr_syn + ":" + \
        cldf.start_syn.map(str) + "-" + cldf.end_syn.map(str)
    cldf["syn_len"] = cldf.end_syn - cldf.start_syn

    # add age categories
    syn_gen_bkgd_file = "/dors/capra_lab/projects/enhancer_ages/hg19_syn_gen_bkgd.tsv"
    syn_gen_bkgd = pd.read_csv(syn_gen_bkgd_file, sep='\t')  # read the file
    syn_gen_bkgd[["mrca", "mrca_2"]] = syn_gen_bkgd[[
        "mrca", "mrca_2"]].round(3)  # round the ages

    # lump Euarchontoglires w/ Boreotheria. Group is too small
    syn_gen_bkgd.loc[syn_gen_bkgd["mrca_2"] == 0.131, "mrca_2"] = 0.152
    syn_gen_bkgd.loc[syn_gen_bkgd["taxon2"]
                     == "Euarchontoglires (90)", "taxon2"] = "Boreotheria (96)"

    syn_gen_bkgd = syn_gen_bkgd[["mrca", "taxon",
                                 "mrca_2", "taxon2"]]  # whittle down the df
    ...
    active_cldf["act/synlen"] = active_cldf.activity.divide(
        active_cldf.syn_len)

    wtact = cldf.groupby(["syn_id", "syn_len", "arch"])[
                         "activity"].max().reset_index()
...
def match_len(core_df, der_df, base_len):
    ...
    core.matching_len = core.matching_len.astype(float).apply(
        lambda x: custom_round(x, base=base_len))  # round to the nearest 100bp

    der = der_df[columns].drop_duplicates()
    der.columns = columns_names

    der.matching_len = der.matching_len.astype(float).apply(
        lambda x: custom_round(x, base=base_len))

    # find the intersecting lengths
    lens = set(list(core.matching_len.unique())
               + list(der.matching_len.unique()))
    ...
    for length in lens:
        ...
        if length > 0 and sn > 0:
            ...
            der_ids = der.loc[der.matching_len == length].sample(
                n=sn, replace=True)  # sample w/ replacement
            core_ids = core.loc[core.matching_len == length].sample(
                n=sn, replace=True)  # sample w/ replacement
    ...
    for mrca_2 in df.mrca_2.unique():
        for arch in df.core_remodeling.unique():
            ...
            emptydf = pd.DataFrame(
                {"mrca_2": [mrca_2], "core_remodeling": [arch], })
...
def get_counts(df, groupby_cols, groupby_val):
    ...
    if "mrca_2" in groupby_cols and "core_remodeling" in groupby_cols:
        ...
        # make an empty df to fill architectures w/ no counts
        empty = make_empty_dict(info_df)
    ...
    # only keep the core and derived counts
