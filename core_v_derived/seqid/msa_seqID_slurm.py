...
def extract_pairwise_fa_data(fa_handle, dist, chr_):
    ...
    with open(fa_handle, "r") as fa_reader:
        ...
        hg19_seq, current_seq, oldest_seq = "", "", "" # keep track of hg19, current, and oldest sequences
        
        husize, current_size, oldest_size = 0, 0, 0

        current_species, oldest_species = None, None  # keep track of current species
        
        current_mrca, oldest_mrca = 0, 0  # keep track of the oldest MRCA age
        ...
        ref_species = [
                        ...
                        "ponAbe2","rheMac2",
                        #"papHam1",
                        "calJac1","tarSyr1","otoGar1",# mouse lemur on same MRCA. 
                        "tupBel1","mm9",
                        "canFam2","loxAfr3","macEug1",
                        "monDom5","ornAna1","galGal3",
                        "xenTro2","danRer6","petMar1"
                        ]
        ...
        for i, line in enumerate(fa_reader):
            ...
            if ">" in line:
                ...
                if list(set(current_seq)) != ['*']:
                    ...
                    if current_seq !="" and current_species in ref_species:
                        ...
                        if current_mrca not in seqid_dict.keys():
                            seqid_dict[current_mrca]=[percent]
                            
                        else:
                            val = seqid_dict[current_mrca] # append percent to current list
                            val.append(percent)
                            seqid_dict[current_mrca] = val 
                            
                    # check whether the last species is older
                    if current_mrca > oldest_mrca and current_species in ref_species:
                    
                        oldest_mrca = current_mrca  # if so, reassign oldest age
                        oldest_species = current_species  # reassign oldest species
                        oldest_seq = current_seq  # reassign oldest sequence. 
                        oldest_size = current_size
                        oldest_percent = percent
                    
                """
                    else:
                        print("not older", current_species, current_mrca)
                else:
                    print("not alignment", current_species, current_mrca)
                """     
                # begin new species
                ...
                #print(current_species, current_mrca)
                n+=1
    ...
    #print(seqid_dict,"\n\n", oldest_species, oldest_mrca, oldest_seq)
    
    # make dataframe of ages and sequence identity scores
    newdf = pd.DataFrame({
                "MRCA":seqid_dict.keys(),
                "seqids":seqid_dict.values()
                 })

    # Median sequence ID score for ages with multiple sequence scores
    #newdf["med_seqid"] = newdf["seqids"].apply(lambda x: np.median(x))
    
    # SD sequence ID score for ages with multiple sequence scores
    #newdf["SD_seqid"] = newdf["seqids"].apply(lambda x: np.std(x))

    newcols = ["region_id", "oldest_species", "oldest_mrca", "oldest percent"]
    newdf[newcols] =region_id, oldest_species, oldest_mrca, oldest_percent

    newdf.sort_values(by="MRCA")
    newdf = newdf.loc[newdf["MRCA"]==oldest_mrca]
    
    return newdf[newcols], region_id

def quantify_fa_seqids(chr_, data_path, distance):
    ...
    if os.path.exists(out) is False and len(fa_handles)>0:
        ...
        for f in fa_handles:
            seqid_df, region_id= extract_pairwise_fa_data(f, dist, chr_)
