...
vals = {"enh_path": "/dors/capra_lab/projects/enhancer_ages/fantom/data/download/CL_0000094_granulocyte_expressed_enhancers/non-genic/no-exon_CL_0000094/ages",
"arens_path":"/dors/capra_lab/data/mpra/arensbergen18/",
"outdata_path": "/dors/capra_lab/projects/enhancer_ages/arensbergen2019/data/",
"sid": "CL_0000094_granulocyte_expressed_enhancers"}

vals["enh"] = "%s/syn_breaks_no-exon_CL_0000094_ages.bed"% vals["enh_path"]
...
#%%
# 2187 enhancers overlap arensbergen of 668 total enhancers)
...
n_sig_simple_enhancer = len(overlap.loc[(overlap.core_remodeling.astype(int) ==0)\
 & (overlap.sig_fdr05.astype(int) > 0 )])
...
n_sig_complex_core = overlap.loc[(overlap.core_remodeling.astype(int) ==1)\
 & (overlap.sig_fdr05.astype(int) > 0 ) &(overlap.core.astype(int) ==1)]["syn_id"].count()

 # sig SNPs in complex cores
n_sig_complex_der = overlap.loc[(overlap.core_remodeling.astype(int) ==1)\
 & (overlap.sig_fdr05.astype(int) > 0 ) &(overlap.core.astype(int) ==0)]["syn_id"].count()
...
overlap.loc[(overlap.core_remodeling.astype(int) ==1) & (overlap.sig_fdr05.astype(int) > 0 )]
...
sig_no_overlap = len(df.loc[(df.overlap.astype(int) == 0) & (df.sig_fdr05.astype(float) == 1)])
