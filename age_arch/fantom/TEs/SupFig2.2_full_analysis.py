...
datetime.now())
#%%
RE ="/dors/capra_lab/projects/enhancer_ages/fantom/results/for_publication/age_breaks/no_repeatmasker/"
...
enh = "%sFANTOM_NOREPEATMASKER_enh_age_arch_full_matrix.tsv" % path
summaryEnh = "%sFANTOM_NOREPEATMASKER_enh_age_arch_summary_matrix.tsv" % path

shuf = "%sSHUFFLE_NOREPEATMASKER_FANTOM_enh_age_arch_full_matrix.tsv" % path
summaryShuf = "%sSHUFFLE_NOREPEATMASKER_FANTOM_enh_age_arch_summary_matrix.tsv" % path
...
sns.barplot(x = "taxon2", y  ="total_freq", data = lens_dict["no-repeats_shuf-all_fantom_enh_112_tissues-54_age_breaks"].sort_values(by="mrca_2"), palette = sg_p, ax=ax2)
...
print("Simple enhancer slope = %s, r2 = %s, pval = %s\nComplex enhancer slope = %s, r2 = %s, pval = %s"
             % (round(slope_s, 3), round(r_value_s,2), p_value_s, round(slope_c,3),  round(r_value_c,2), p_value_c))
print("SHUFFLE Simple enhancer slope = %s, r2 = %s, pval = %s\nSHUFFLE Complex enhancer slope = %s, r2 = %s, pval = %s"
              % (round(slope_ss, 3), round(r_value_ss,2), p_value_ss, round(slope_cs,3),  round(r_value_cs,2), p_value_cs))
...
len_concat.datatype2.unique()
#%%
for dataset in len_concat.datatype2.unique():
    print(dataset)

    test = len_concat.loc[len_concat.datatype2 == dataset]
    sns.distplot(test.enh_len, label = dataset)
plt.legend()
#%%
# In[75]:
...
sns.barplot(x = "taxon2", y = "seg_index",
            data =plot_breaks[(plot_breaks.core_remodeling ==1) & (plot_breaks.taxon2!= "Homo sapiens (0)")].sort_values(by = "mrca_2"),
...
syn_counts= final_merge.groupby(["mrca", "arch"])["enh_id"].count().reset_index()

# group by architecture
totals = syn_counts.groupby(["arch"])["enh_id"].sum().reset_index()
totals.columns = ["arch", "total_count"]

# calculate age v. architecture
concat2 = pd.merge(syn_counts, totals, how = "left", on = "arch")
...
#####SHUFFLE#####
# group by age & architecture
shuf_arch= shuffle.groupby(["mrca", "arch"])["enh_id"].count().reset_index()

# group by architecture
shuf_totals = shuf_arch.groupby(["arch"])["enh_id"].sum().reset_index()
shuf_totals.columns = ["arch", "total_count"]

# calculate age v. architecture
shuf_concat = pd.merge(shuf_arch, shuf_totals, how = "left", on = "arch")
...
#####CONCAT2#####
concat_bkgd2["arch2"] = concat_bkgd2["arch"] + "-"+ concat_bkgd2["dataset"]
...
df_empty = pd.DataFrame({'mrca' :[0.957, 0.957, 0.000, 0.000], 'code':["derived", "derived", "complex_core", "complex_core"],
 ...
 'arch2':["derived-FANTOM", "derived-shuffle", "complex_core-FANTOM", "complexenh-shuffle"]})
concat_bkgd2 = concat_bkgd2.append(df_empty, ignore_index=True)
concat_bkgd2.sort_values(by = "mrca").head()

tx2 = concat_bkgd2.groupby(['mrca_2', 'arch', 'taxon2', 'dataset' , "arch2"])["frequency"].sum().reset_index()
...
matharch.loc[(matharch.arch.str.contains("simple")) & (matharch.mrca_2<0.175)]["frequency"].sum()
...
pivot2 = pd.pivot_table(tx2, values='frequency',
 index=['mrca_2', 'arch', 'taxon2'], columns=['dataset']).reset_index()
...
fig2e_plot = tx2.loc[(tx2["dataset"].str.contains("FANTOM"))].sort_values(by=["mrca_2", "arch"] )
#fig2e_plot= fig2e_plot.loc[(fig2e_plot.taxon2 != "Homo") & (arch.str.contains("complex") )]
#####COMPLEX ENH#####
order = ["simple", "complexenh"]
...
sns.barplot(x = "taxon2", y = "frequency",
              data = fig2e_plot, hue = "arch",palette = arch_palette, hue_order = order)
ax.set_xticklabels(ax.get_xticklabels(),
                    rotation=90, horizontalalignment = "left", fontsize = 20)

mwu_stat, pval = stats.mannwhitneyu(final_merge["mrca_2"].loc[(final_merge["arch"].str.contains("complexenh"))],
                  final_merge["mrca_2"].loc[(final_merge["arch"].str.contains("simple"))])
...
arch_palette = sns.xkcd_palette(arch_colors)
sns.barplot(x = "taxon2", y = "frequency",
              data = tx2.loc[tx2["arch"].str.contains("complexenh")].sort_values(by=["mrca_2", "arch"]),\
              hue = "arch2", ax = ax2,  palette = arch_palette)
ax2.set_xticklabels(ax2.get_xticklabels(), rotation=90, horizontalalignment = "left", fontsize = 20)
mwu_stat, pval = stats.mannwhitneyu(final_merge["mrca_2"].loc[(final_merge["arch"].str.contains("complexenh"))],
                   shuffle["mrca_2"].loc[(shuffle["arch"].str.contains("complexenh"))])
...
arch_palette = sns.xkcd_palette(arch_colors)
sns.barplot(x = "taxon2", y = "frequency",
              data = tx2.loc[tx2["arch"].str.contains("simple")].sort_values(by=["mrca_2", "arch"]),\
              hue = "arch2", ax = ax3,  palette = arch_palette)

ax3.set_xticklabels(ax3.get_xticklabels(), rotation=90, horizontalalignment = "left", fontsize = 20)

mwu_stat, pval = stats.mannwhitneyu(final_merge["mrca_2"].loc[(final_merge["arch"].str.contains("simple"))],
                   shuffle["mrca_2"].loc[(shuffle["arch"].str.contains("simple"))])
...
sns.barplot(x = "taxon2", y = "fold_change",
data = pivot2.loc[pivot2["arch"].str.contains("complexenh")].sort_values(by=["mrca_2", "arch"]),
              hue = "arch", ax = ax2,  palette = complexcorep)
ax2.set_xticklabels(ax2.get_xticklabels(), rotation=90,
...
mwu_stat, pval = stats.mannwhitneyu(final_merge["mrca"].loc[(final_merge["arch"].str.contains("complexenh"))],
                   shuffle["mrca"].loc[(shuffle["arch"].str.contains("complexenh"))])
...
sns.barplot(x = "taxon2", y = "fold_change",
data = pivot2.loc[pivot2["arch"].str.contains("simple")].sort_values(by=["mrca_2", "arch"]),
hue = "arch", ax = ax3,  palette = simplep)
...
mwu_stat, pval = stats.mannwhitneyu(final_merge["mrca"].loc[(final_merge["arch"].str.contains("simple"))],
shuffle["mrca"].loc[(shuffle["arch"].str.contains("simple"))])
print("simple", mwu_stat, pval)
