...
samples = glob.glob("%sno-exon_FANTOM_enh_age_arch_summary_matrix.bed" % path)
...
def formatDF(df, datatype):
    ...
    #df = pd.merge(df, syn_gen_bkgd,\
     #how = "left", on = "mrca" ).sort_values(by="mrca")# add species annotation
...
out_shuf = "%sSHUFFLE_NOEXON_FANTOM_enh_age_arch_full_matrix.tsv" % path
...
out_enh_shufBreaks = "%sSHUFFLE_NOEXON_FANTOM_enh_age_arch_summary_matrix.tsv" % path
...
for sample in samples:
    if os.path.getsize(sample)>0:
        ...
        df.columns = ["chr_enh", "start_enh","end_enh", "enh_id",
                       ...
                       "enh_len", "taxon", "mrca_2", "taxon2",
                        "mya", "mya2", "seg_den", "datatype"]
