...
def get_relative_simple(sid, pdf):
    ...
    if sid in pdf.sid2.unique():
        ...
        relative_simple = pdf.loc[(pdf.sid2 == sid) &\
         (pdf.pct < 0.5), "seg_index"].max().astype(int)
...
for sid in devsid_list:
    ...
    multipath = "/dors/capra_lab/projects/enhancer_ages/roadmap_encode/data/hg19_roadmap_samples_enh_age/download/h3k27ac_plus_h3k4me3_minus_peaks/"
    multifile = "%sbreaks/multiintersect/trim-%s_multiintersect_0.5_count.bed"% (multipath,  sid) # all 98 tissues
    ...
    agefile = "%sbreaks/ROADMAP_%s_enh_and_shuf_age_arch_summary_matrix.bed"% (multipath,  sid)
    if os.path.exists(multifile)==True:
        print(sid)
        enh = pd.read_csv(agefile, sep = '\t', header = None, usecols=[3, 4, 6, 7, 9, 13, 12])


        enh.columns =[ "dataset",  "old_enh_id","arch", "seg_index", "old_len", "taxon2", "mrca_2"]

        # get relative percentiles and reassign "arch"
        enh_percentiles = "%sbreaks/all_ROADMAP_breaks_percentiles.bed" % multipath
