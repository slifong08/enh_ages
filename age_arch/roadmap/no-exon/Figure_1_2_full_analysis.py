...
RE ="/dors/capra_lab/projects/enhancer_ages/roadmap_encode/results/age_breaks/non-genic/"
...
enh= "%snon-genic/no-exon_ROADMAP_E022_enh_and_shuf_age_arch_summary_matrix.bed" % (path)
...
# count of simple and complex enhancers in E022
...
sns.set_style("white")
plt.savefig("%sE022_enh_shuf_freq.pdf" %RE, bbox_inches = "tight")
...
ax.set_xlabel("")
ax.set_ylabel("Frequency of Dataset")
ax.set_title("E022 Enhancer Architectures")
for p in ax.patches:
    x=p.get_bbox().get_points()[:,0]
    y=p.get_bbox().get_points()[1,1]
    ax.annotate('{:.0f}%'.format(100.*y), (x.mean(), y),
...
ax.set_title("")
#ax.get_legend().remove()
plt.savefig("%sE022_enh_arch_freq.pdf" %RE, bbox_inches = "tight")
...
breaks_freq["dataset"] = "E022"
breaks_freq["shuf_id"] = "E022"
...
xlim = (0,10))
plt.savefig("%sE022_CDF.pdf" %RE, bbox_inches = 'tight')
...
firstfive = ORdf.loc[ORdf.seg_index <15]
sns.barplot(x = "seg_index", y = "log", data = firstfive.loc[firstfive.seg_index <15],
...
plt.savefig("%sfig2b-E022_age_seg_fold_change_matplotlib.pdf" %RE, bbox_inches = "tight")
...
lens_dict["E022"] = get_age_arch_freq(lens, "datatype", "E022")
lens_dict["E022"].dataset.unique()
...
ax.set_title("")
#ax.get_legend().remove()
plt.savefig("%sE022_enh_lens.pdf" %RE, bbox_inches = "tight")
...
stats.mannwhitneyu(lens.loc[(lens.mrca_2>0.175) & (lens.datatype == "E022"),"enh_len"],
                  lens.loc[(lens.mrca_2<=0.175) & (lens.datatype == "E022"),"enh_len"],)
...
ax.set_title("")
ax.get_legend().remove()
plt.savefig("%sfig1d-E022_enh_mrca_lens.pdf" %RE, bbox_inches = "tight")


# In[66]:


stats.mannwhitneyu(lens.loc[(lens.arch == "simple") & (lens.datatype == "E022"), "enh_len"],
...
print("Simple enhancer slope = %s, r2 = %s, pval = %s\nComplex enhancer slope = %s, r2 = %s, pval = %s"
             % (round(slope_s, 0), round(r_value_s,2), p_value_s, round(slope_c,0),  round(r_value_c,2), p_value_c))
print("SHUFFLE Simple enhancer slope = %s, r2 = %s, pval = %s\nSHUFFLE Complex enhancer slope = %s, r2 = %s, pval = %s"
             % (round(slope_ss, 0), round(r_value_ss,2), p_value_ss, round(slope_cs,0),  round(r_value_cs,2), p_value_cs))
...
plt.savefig("%sfig2c-E022_ENH_MRCA_x_LEN_ENH.pdf" % RE, bbox_inches = "tight")
...
e_pal = sns.xkcd_palette(e_colors)
hue_order = ['E022-simple','SHUFFLE-simple','E022-complexenh','SHUFFLE-complexenh']
...
ax1.set_title("")
ax1.legend().remove()
plt.savefig("%sfigS2.7-E022_ENH_MRCA_x_LEN_ENH.pdf" % RE, bbox_inches = "tight")
...
ax.set_title("E022 age segments v MRCA")
ax.set_xlabel("")
ax.set_title("")
ax.get_legend().remove()
ax.ylim = (0,3.2)
plt.savefig("%sFigS2.4B-E022_age_segments_v_MRCA_bar.pdf" %RE, bbox_inches = "tight")
...
ax.set_xticklabels(ax.get_xticklabels(), rotation = 90, horizontalalignment = "left")
ax.set_ylabel("# age segments")
ax.set_title("E022 v Shuffle age segments")
ax.set_xlabel("")
ax.set_title("")
ax.legend().remove()
ax.ylim = (0,3.2)
plt.savefig("%sFigS2.4A-E022_age_segments_bar.pdf" %RE, bbox_inches = "tight")
...
stats.mstats.kruskalwallis(*argsF) # KW on age segment differences among E022 complex enhancers
...
# MWU on age segments in E022 v. Shuffle
stats.mannwhitneyu(plot_breaks.loc[(plot_breaks.datatype == "E022") & (plot_breaks.core_remodeling ==1), "seg_index"],
...
plot_dist = final_merge.groupby("enh_id")["mrca_2"].max().reset_index() # calculate age distribution of E022 enhancers by enh_id
...
plot_dist["shuf_id"] = "E022"
plot_dist["dataset"] = "E022"
dist = pd.concat([shuffle_dist2, plot_dist]) # concat shuffle and E022 age distributions
dist.head()
...
# calculate MWU shuffle v. E022 age distributions (from each enhancer)

mwustat, mwupval = stats.mannwhitneyu(shuffle_dist2["mrca_2"].loc[shuffle_dist2["dataset"] =="shuffle"],
                                     plot_dist["mrca_2"].loc[plot_dist["dataset"] =="E022"] )
...
# Fold change of E022/shuffle age frequency
fold_change_dict = {}
for i in dist.shuf_id.unique():
    if i != "E022":
        fold_change = dist.loc[(dist.shuf_id.str.contains(i))|(dist.shuf_id == "E022")].pivot(index="mrca_2", columns='dataset')['freq'].reset_index()
        fold_change = pd.merge(fold_change, syn_gen_bkgd[["mrca_2", "taxon2"]], how = "left").drop_duplicates()
        fold_change["fold_change"] = np.log2(fold_change["E022"].divide(fold_change["shuffle"]))
...
ax2.set_ylim(0, 0.6)
ax2.set_title("E022 Enhancer Age Distribution")
ax2.set_xlabel("")
ax2.set_title("")
...
ax1.set_ylim(-2.7, 2)
ax1.set_title("E022 Enhancer Age Fold-change")
ax1.set_xlabel("")
ax1.set_title("")

plt.savefig("%sfig1b_1c-E022_WHOLE_ENHANCER_MRCA_DIST_TAXON2.pdf" % (RE), bbox_inches='tight')
...
m, mp = stats.mannwhitneyu(plot_ages.loc[plot_ages.data_type == "shuffle", "mrca_2"],
                        plot_ages.loc[plot_ages.data_type == "E022", "mrca_2"])
print(m, mp)
...
syn_counts= final_merge.groupby(["mrca", "code"])["enh_id"].count().reset_index()

# group by architecture
totals = syn_counts.groupby(["code"])["enh_id"].sum().reset_index()
totals.columns = ["code", "total_count"]

# calculate age v. architecture
concat2 = pd.merge(syn_counts, totals, how = "left", on = "code")
concat2["frequency"] = concat2["enh_id"].divide(concat2["total_count"])
concat2["dataset"] = "E022"


#####SHUFFLE#####
# group by age & architecture
shuf_arch= shuffle.groupby(["mrca", "code"])["enh_id"].count().reset_index()

# group by architecture
shuf_totals = shuf_arch.groupby(["code"])["enh_id"].sum().reset_index()
shuf_totals.columns = ["code", "total_count"]

# calculate age v. architecture
shuf_concat = pd.merge(shuf_arch, shuf_totals, how = "left", on = "code")
shuf_concat["frequency"] = shuf_concat["enh_id"].divide(shuf_concat["total_count"])
...
df_empty = pd.DataFrame({'mrca' :[0.957, 0.957, 0.000, 0.000], 'code':["derived", "derived", "complex_core", "complex_core"],
 ...
 'total_count':[0, 0, 0, 0],
 'frequency':[0, 0, 0, 0],
 'dataset':["E022", "shuffle", "E022", "shuffle"],
 'taxon':["Vertebrata (615)", "Vertebrata (615)", "Homo sapien (0)", "Homo sapien (0)"],
 'mrca_2':[0.957, 0.957, 0.000, 0.000],p
 'taxon2':["Vertebrata (615)", "Vertebrata (615)", "Homo sapien (0)", "Homo sapien (0)"],
 'code2':["derived-E022", "derived-shuffle", "complex_core-E022", "complex_core-shuffle"]})
...
matharch = tx2.loc[tx2.dataset =="E022"].drop_duplicates()
matharch
...
plt.savefig("%sMRCA_E022_v_shuffle_core_derived_grouped2.pdf"% (RE))
...
pivot2["fold_change"] = pivot2["E022"].divide(pivot2["shuffle"]) -1
...
plt.savefig("%sE022_v_shuffle_mrca_arch_fold_change_tx2.pdf"% (RE), bbox_inches = "tight")


# In[133]:


tx2.loc[(tx2["dataset"].str.contains("E022")) &( tx2.code.str.contains("complex")| tx2.code.str.contains("simple"))].sort_values(by=["mrca_2", "code"] )
...
fig2e_plot = tx2.loc[(tx2["dataset"]                      .str.contains("E022"))                      &( tx2.code.str.contains("complex")                       | tx2.code.str.contains("simple"))]                    .sort_values(by=["mrca_2", "code"] )
...
plt.savefig("%sfig2e-MRCA_E022_complex_v_simple.pdf"% (RE), bbox_inches = "tight")
...
plt.savefig("%sfigS2-MRCA_E022_v_shuffle_core_derived_grouped_enh.pdf"% (RE), bbox_inches = "tight")
...
pivot2 = pd.pivot_table(tx2, values='frequency',                           index=['mrca_2', 'code', 'taxon2'], columns=['dataset']).reset_index()
pivot2["fold_change"] = np.log2(pivot2["E022"].divide(pivot2["shuffle"]))
pivot2=pivot2.fillna(0)
...
plt.savefig("%sfigS2e-E022_v_shuffle_mrca_arch_fold_change_tx2_enh.pdf"% (RE), bbox_inches = "tight")
