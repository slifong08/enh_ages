...
dflen = df["enh_len"].to_list()
slen = shufdf["enh_len"].to_list()
np.median(dflen)
np.median(slen)
stats.mannwhitneyu(dflen, slen)
...
catdf["enh_len"].describe()
catdf.groupby(["id"])["enh_id"].count()
outf = f"{ENHPATH}all_age_arch.bed"
catdf.dataset_name.unique()
outdf = catdf.drop(["species_specific", "id"], axis = 1)
outdf.to_csv(outf, sep = '\t', index = False)
list(outdf)
#%%
# basic info
