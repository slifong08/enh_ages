...
fs = glob.glob("%sGSM*_Hu_*_H3K27ac*.bed" % (path))
...
def get_consensus(devtime, dataset_id, path):
    ...
    id_num_b = id_num+1
    consensus_f = "%sconsensus/consensus_hu_H3K27ac_%s.bed" %(path, devtime)

    if devtime == "Hu_8-5": # fancy formatting for this 8.5pcw
        devtime = "8_5pcw"

    a = "%s%s_Hu_%s_H3K27ac_rep1_regions.bed" % (path, dataset_id, devtime)


    b = "%sGSM%s_Hu_%s_H3K27ac_rep2_regions.bed" % (path,id_num_b, devtime)
...
def format_df(f):
    ...
    sid = (f.split("/")[-1]).split(".")[0]#"GSM1554668_Hu_8_5pcw_H3K4me2_rep1_regions.bed"

    dataset_id = (sid.split("_")[0])


    if "Hu_8_5" in sid: # resolve filename differences.
        rep = sid.split("_")[5]
        devtime = "Hu_8-5"
...
outf = "%sconsensus/consensus_hu_H3K27ac_allpcw.bed" % path
