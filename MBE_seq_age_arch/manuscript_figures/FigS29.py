...
#%% Files
ARCH_STATS = glob.glob(f"{PATH}no-exon_E*_ARCH_metrics.txt")
FREQS = glob.glob(f"{PATH}no-exon_E*_id_mrca_2_freq.txt")
FCS = glob.glob(f"{PATH}no-exon_E*_id_mrca_2_freq.txt")

ARCH_STATS = glob.glob(f"{PATH}no-exon_E*_AGE_ARCH_metrics.txt")
ARCH_FREQS = glob.glob(f"{PATH}no-exon_E*_id_arch_mrca_2_freq.txt")
ARCH_FCS = glob.glob(f"{PATH}no-exon_E*_id_arch_mrca_2_fold_change.txt")

MRCA_OR = glob.glob(f"{PATH}no-exon_E*_summary_age_arch_OR.txt")
SEG_OR = glob.glob(f"{PATH}no-exon_E*_summary_seg_OR.txt")
...
def plot_figS2xC_len_mrca(arch, fig_id, roadmap_id, countdf, groupby_val, order, hue, palette):
    ...
    fig, ax = plt.subplots(figsize =(6,6))
    splot = sns.barplot(x=x, y=y, data = data, hue = hue,
    palette = palette, hue_order = order)

    ax.set(ylabel = "length (bp)", xlabel = "Sequence Age",
    title = roadmap_id, ylim = (0,3000))
    ax.set_xticklabels(xlabs, rotation = 90)
    ax.legend(bbox_to_anchor = (1,1))
...
#%%
first_five = df.loc[df.seg_index<10]
