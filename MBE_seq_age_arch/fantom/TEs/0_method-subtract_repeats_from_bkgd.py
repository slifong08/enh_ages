...
RepeatP = "/dors/capra_lab/projects/enhancer_ages/te/data/"
RepeatF = "%srepeatmasker_hg19.bed" % RepeatP
...
def bed_subtract(shufF, RepeatF, outP):

    fName = (shufF.split("/")[-1]).split(".")[0]
    outF = "%sno-repeatmasker_%s.bed" % (outP, fName)

    # use -v argument to subtract exons from shuffle file.
    cmd = "bedtools intersect -a %s -b %s -v > %s" % (shufF, RepeatF, outF)
...
for ShufF in ShufFs:
    ...
    bed_subtract(ShufF, RepeatF, ShufOutP)
...
for EnhF in EnhFs:
    ...
    bed_subtract(tempF, RepeatF, OutP)
