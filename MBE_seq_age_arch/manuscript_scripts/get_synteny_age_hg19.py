...
# the purpose of this graph is to take bed files of
# synteny blocks from 46-way multiZ HG19 multiple sequence alignments
# and assign ages to each of those synteny blocks
# based on the maximum patristic phylogenetic distance between nodes,
...
phdf.columns = ["taxon1", "taxon2", "patr_dist", "edge_dist",
"taxon1_mrca_dist", "taxon1_mrca_edge_dist",
"taxon2_mrca_dist", "taxon2_mrca_edge_dist"]
...
def enh_age(file):
    ...
    df.columns = ["chr", "start", "end", "strand", "taxon1",
     "species_count", "taxon2"]

    df["len"] = df["end"]- df["start"]

    # parse through the species-overlap list,
    # created a matrix of species overlap and
    # phylogenetic values in order to find the oldest enhancer age.
    ...
    df= df[["chr", "start", "end", "strand", "taxon1",
    "species_count", "len", "max_mrca","max_patr"]] # chr, start, end, overlap number, sample_id, multiz start, multiz end, number of species, length, furtherst phylogenetic distance.
